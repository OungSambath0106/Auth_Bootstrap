    ...
    @section('content')
        ...
        <div class="list-group w-auto p-3 mt-1" style="border-radius: 10px">
            <div class="list-group-item" style="background-color: #3559E0" aria-current="true">
                <h4 style="color: #FFFFFF;" class="mt-2"><b>MenuType List</b></h4>
            </div>
                    ...
                    <table id="example1" class="table">
                        ...
                        <tbody class="tbody">
                            @foreach ($menutypes as $menutype)
                                <tr>
                                    <td class="px-3" style="padding-top: 12px; scope="row"> {{ $menutype->id }} </td>
                                    <td class="px-3" style="padding-top: 12px; scope="row"> {{ $menutype->name }} </td>
                                    <td class="px-3" scope="row">
                                        @can('update menutype')
                                            <a href="{{ url('menutypes/' . $menutype->id . '/edit') }}" type="button" class="btn edit"
                                        ...
                                        @can('delete menutype')
                                            <a class="btn trash" href="#"
                                                onclick="event.preventDefault(); confirmDelete({{ $menutype->id }})"
                ...
                swalWithBootstrapButtons.fire({
                    ...
                    if (result.isConfirmed) {
                        swalWithBootstrapButtons.fire({
                            ...
                            window.location.href = "{{ url('menutypes') }}/" + roleId + "/delete";
                        });
                    }
                });
            }
            ...
            $(function() {
                $("#example1").DataTable({
                        ...
                        @can('create menutype')
                            {
                                ...
                                action: function() {
                                    window.location.href = "{{ url('menutypes/create') }}";
                                }
                            }
                        @endcan
                    ],
                    ...
                    "dom": // Custom DOM layout with search input on the left and buttons on the right
                        "<'row mt-2 mb-2 px-3'<'col-md-6 d-flex justify-content-between'f><'col-md-6'B>>" +
                        "<'row'<'col-md-12'tr>>" +
